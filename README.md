Description
AI-assisted coding is everywhere now — by 2026, most developers have at least tried it. A few practical truths have become clear:
It’s a hands-on skill. You don’t learn it from articles — you learn it by coding. It’s deeper than it looks. In practice, it can take 1–2 months and roughly 10–20K lines of code before the workflow starts to feel natural. Not every approach works equally well. The tools evolve quickly, so what worked six months ago may already feel outdated.
We’ll share what’s working for us today and support those who want to build practical AI coding habits — the kind you actually use in day-to-day development.
The good news: these skills transfer across most modern AI coding assistants. If you already use one, bring it. If not, we’ll suggest a few options — nothing is mandatory.
How It Works
We start with a practical overview: what works today, common pitfalls, and recommended workflows. You will receive a week-long coding assignment. During the week, we meet regularly to discuss progress, questions, and different approaches. At the end, we review what you built and share how we evaluate the results.
Participant Requirements
You should already be comfortable programming in any industry-level language.
By this we mean:
•	at least one year of experience, and
•	roughly 50K lines of code written.
Experience building a UI (web, mobile, or desktop) is helpful — the assignment will include a visual component.
Required Software & Setup
Please have:
•	your usual IDE,
•	an AI coding assistant (if you don’t have one already, consider setting one up — trial versions may be limited),
•	20–40 hours during the week to focus on coding.
 
Assignment Instruction
1.	Complete the task from the workshop.
2.	Create public GitHub or GitLab repository with your project.
3.	Project MUST be buildable and runnable by "docker compose up" in the root repository folder.
4.	Publish your solution on GitHub or GitLab and make sure your repository is set to public.
...
The user shall be able to view a list of their active sessions, including browser/IP details, and log out selected sessions. Logging out from the current browser invalidates only the session for that browser. Other active sessions remain valid until the user explicitly logs them out from this screen or they expire naturally.
...
Maximum text size per message: 3 KB. Message text must support UTF-8.
2.5.3. Message Replies
A user may reply to another message. The replied-to message shall be visually outlined or quoted in the message UI.
2.5.4. Message Editing
Users shall be able to edit their own messages. If a message was edited, the UI shall display a gray “edited” indicator similar to common chat applications.
...
The system shall preserve the original file name. The user may add an optional comment to an attachment.
2.6.4. Access Control
Files and images may be downloaded only by current members of the chat room or by authorized participants of the personal chat. If a user loses access to a room, they must also lose access to the room’s files and images.
...
•	a chat room
•	a personal dialog
the UI shall show a notification indicator near the corresponding room or contact. The unread indicator is cleared when the user opens the corresponding chat.
